...
public class ProjectAF {
	...
	public Action getAction(String command) {
		...
		if (command.equals("projectjoinaccept")) {
			action = new GoToProjectJoinAcceptAction();
		}
		if (command.equals("acceptorrejectproc")) {
			action = new ProjJoinAcceptOrRejectProcessAction();
		}
		if (command.equals("putfreelancer")) {
			action = new GoToPutFreelancerAction();
		}
		if (command.equals("addtoputinfreelancerlist")) {
			action = new AddToPutInFreelancerListAction();
		}
		if (command.equals("gotoputinofsearchproject")) {
			action = new GoToPutInOfSearchProjectAction();
		}
		if (command.equals("putrequestordelete")) {
			action = new GoToPutRequestOrDeleteAction();
		}
		if (command.equals("sendmessageforputinrequestproc")) {
			action = new SendMessageForPutInProcessAction();
		}
		if (command.equals("sendmessageandputinproc")) {
			action = new SendMessageAndPutInProcessAction();
		}
		////////////////////////////////////////////////////////////////////////////////////////////////////
		return action;
	}
